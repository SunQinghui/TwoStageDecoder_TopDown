...
class TextDataset(DatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
        ...
        # out_examples is a list, which contains the pair of data between source and target.
    ...
    def _dynamic_dict(self, examples_iter):
        for example in examples_iter:
            src = example["src"]
            # 将一段话按不重复的词排序，然后建立索引[1 - len]。
            src_vocab = torchtext.vocab.Vocab(Counter(src),
                                              specials=[UNK_WORD, PAD_WORD])
            self.src_vocabs.append(src_vocab)
            # Mapping source tokens to indices in the dynamic dict.
            # src_map 和 一段话的单词数量一样长，存的是每个单词在索引中的位置。eg.[9(在上面排序中排第九的单词),10,3,4,....]
            src_map = torch.LongTensor([src_vocab.stoi[w] for w in src])
            example["src_map"] = src_map

            if "tgt" in example:
                tgt = example["tgt"]
                # 存的和tgt原本长度一样长的数字，数字代表tgt中该单词在src_vocabs中的倒排索引。
