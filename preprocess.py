...
def build_save_in_shards(src_corpus, tgt_corpus, fields,
    ...
    while not src_iter.hit_end():
        index += 1
        # 取出来后的dataset,里面的主要结构是：['examples':存了很多索引，每个索引里面有alignment,src,src_map，tgt属性]
        # dataset:{examples,src_vocabs}
        # examples:{0:{alignment,indices,src,src_map,tgt},1:{...},2:{...},...}
        # src_vocab :{freqs(是字典，统计每个单词的频率，不重复),itos(正排索引：index:word),stoi(倒排索引：word:index)}
        # alignment 和 src_map都是对照src_vocabs里面的倒排索引表的。alignment 是tgt的倒排索引表示，src_map是src的倒排索引表示
        # src就是原始的句子，tgt就是原始的结果
        # 有一个问题是dataset中的examples和src_vocabs不一样长。已解决，examples中的examples.indices对应src_vocabs中的位置
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if corpus_type == 'train':
        # src_corpus and tgt_corpus is dir
...
def main():
    ...
    # opt.data_type = 'text', src_nfeats = 0 , tgt_nfeats = 0.
    fields = inputters.get_fields(opt.data_type, src_nfeats, tgt_nfeats)

    logger.info("Building & saving training data...")
    # fields:{src,tgt,src_map,alignment,indices}
